...
interface Props {
	...
	/**
	 * Mobile-only: renders a "Back to list" control at the bottom-left of the
	 * card and calls this when pressed, instead of relying on a pin/
	 * background click to deselect (the mobile card isn't anchored to a pin
	 * — see App.tsx).
	 */
	onBack?: () => void;
}
	...
	return (
		<motion.div
			...
			className={cn(
				...
				// Mobile's h-full alone (no root-level gap/justify-between — that
				// now lives one level down, see below) just gives the group below
				// room to stretch and do its own top/bottom split.
				isMobile ? 'h-full' : '',
				isMobile ? '' : 'rounded-[36px] border border-[#e6eaed] bg-white shadow-xl'
			)}
		>
			{/* Two groups (rather than every section as a direct root child) so
			    mobile's justify-between below has exactly two items — the
			    header/title/stats block pinned to the top, the back+website row
			    pinned to the bottom — with all the extra space (from matching
			    the list's height) landing between them instead of getting
			    distributed among every section. Desktop's own gap-[32px]
			    produces the same 32px gap as before between the two groups,
			    since this is its only child either way. */}
			<div
				className={cn('flex w-full flex-col', isMobile ? 'h-full justify-between' : 'gap-[32px]')}
			>
				...
				<div
					className={cn('flex w-full items-center', isMobile ? 'justify-between' : 'justify-end')}
				>
					{onBack ? (
						<button
							type="button"
							...
							className="inline-flex w-fit shrink-0 cursor-pointer items-center gap-[6px] rounded-[9000px] border border-[#e6eaed] bg-[#f4f6f7] px-[14px] py-[10px] text-[12px] font-normal text-[#041c2c] transition-colors duration-200 ease-out hover:bg-[#e6eaed]"
						>
							<svg
								width="4"
								height="6"
					...
					<a
						...
						className="inline-flex shrink-0 items-center gap-[6px] rounded-[9000px] border border-[#82e472] bg-[#44d62c] px-[14px] py-[10px] text-[12px] font-normal text-[#041c2c] transition-colors duration-200 ease-out hover:bg-[#3bc224]"
					>
						Visit website
						<svg
							width="8"
